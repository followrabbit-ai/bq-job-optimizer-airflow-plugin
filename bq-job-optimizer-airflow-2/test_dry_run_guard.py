...
class TestRewriteDetection(unittest.TestCase):
    def test_detects_query_text_change(self):
        ...
        self.assertFalse(
            plugin._optimized_query_was_rewritten(orig, {"query": {"query": "SELECT 1"}})
        )
        self.assertTrue(
            plugin._optimized_query_was_rewritten(
                orig, {"query": {"query": "SET @@reservation='none';\nSELECT 1"}}
            )
        )
...
class TestDryRunGuard(unittest.TestCase):
    ...
    def test_invalid_rewrite_falls_back(self):
        ...
        # A failing dry-run (mis-split syntax error) must return False -> submit the original.
        self.assertFalse(
            plugin._rewritten_query_dry_run_ok(
                self._hook(side_effect=Exception("Syntax error")), optimized, "src-project"
            )
        )
